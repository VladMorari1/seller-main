import { createDocument, getSchema, saveSchema } from "./masterdata";

// const DATA_ENTITY = 'vtexasia_logs_t';
...
export async function createLogsSchema(ctx: any) {
  ...
  try {
    ...
    if (!schema) {
      ...
      //   },
      // });
      await saveSchema(authToken,account)
    }
...
export async function addLog(
  ...
  const result = await createDocument(ctx,log)
